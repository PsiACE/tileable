"""Event-driven orchestration built with contrib flow tile."""
...
def build_order_flow(registry: TileRegistry, collector: list[dict[str, Any]]) -> type[Tile]:
...
def run_order_pipeline(order_ids: list[int]) -> list[dict[str, Any]]:
    ...
    flow_tile = build_order_flow(registry, collector)
    ...
    for order_id in order_ids:
        invoke_tile(
            ...
            EventFlowPayload(
                entry_tile="fetch",
                entry_payload=FetchPayload(order_id=order_id),
                registry=registry,
            ),
            registry=registry,
            services={"collector": collector},
...
def main() -> None:
    for summary in run_order_pipeline([1, 2, 3]):
        print(summary)
